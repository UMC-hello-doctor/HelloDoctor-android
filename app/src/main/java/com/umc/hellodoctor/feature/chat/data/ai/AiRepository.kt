...
class AiRepository @Inject constructor(
        ...
        return try {
            ...
            val request = GeminiGenerateRequest(
                contents = listOf(
                    Content(
                        role = "user",
                        parts = listOf(Part(text = symptom))  // 순수 증상만 전달
                    )
                ),
                ...
                safetySettings = listOf(
                    SafetySetting("HARM_CATEGORY_HARASSMENT", "BLOCK_ONLY_HIGH"),  // MEDIUM 허용
                    ...
                    SafetySetting("HARM_CATEGORY_DANGEROUS_CONTENT", "BLOCK_ONLY_HIGH")  // 중요!
                )
            )
    ...
    suspend fun generateSymptomSummaryWithAI(
        ...
        val result = sendMessageWithHistory(
